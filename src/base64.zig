...
const Base64 = struct {
    ...
    pub fn init() Base64 {
        ...
        return Base64{
            ._table = upper ++ lower ++ number_symbols,
        };
    }
    ...
    pub fn encode(self: Base64, allocator: std.mem.Allocator, input: []const u8) ![]u8 {
        ...
        for (input, 0..) |_, i| {
            ...
            count += 1;
            if (count == 3) {
                // first 6 bits of 1st byte
                out[iout] = self._char_at(buf[0] >> 2);
                // last 2 bits of 1st byte and first 4 bits of 2nd byte
                out[iout + 1] = self._char_at(((buf[0] & 0x03) << 4) + (buf[1] >> 4));
                // last 4 bits of 2nd byte and 2 bits of 3rd byte
                out[iout + 2] = self._char_at(((buf[1] & 0x0f) << 2) + (buf[2] >> 6));
                // last 6 bits of 3rd byte
...
test "test _char_at" {
    const base64 = Base64.init();
    try std.testing.expectEqual(@as(u8, 'c'), base64._char_at(28));
}
